...
a = [0, 10, 20, 30, 40]
a.index(20)  #2  // 리스트에서 특정 값의 인덱스 구하기
a.reverse()  // 리스트에서 요소의 순서를 반대로 뒤집음
a  #[40, 30, 20, 10, 0]
a = [20, 30, 20, 20, 40, 50]
a.count(20)  #3  // 리스트에서 특정 값의 개수를 구함
a = [1, 3, 2, 6, 5]
...
a = [1, 2, 3] 을 전제로 해놓고,
b = a로 적으면 둘이 같은 객체로 통일되어 a is b를 출력해보면 True가 나오고 중간 요소를 하나 변경하면 a와 b모두 함께 변경되지만,
b = a.copy()로 적으면 요소 내용만 복사되고 객체는 서로 다르므로 a is b를 출력해보면 False가 나오고 중간 요소를 하나 변경해도 둘중 하나만 변경된다.
...
a = [i * j for j in range(2, 10)
            for i in range(1, 10)]      // 리스트 표현식에 for가 여러개일때 처리순서가 뒤에서 앞으로 순이라고하여, 이게 강의영상 예시인데,
a = [i * j for i in range(2, 10)
            for j in range(1, 10)]      // 내 생각엔 위 코드말고 이 코드가 맞는 예시가 아닌가 싶다.(아닐수도 있으니 이부분은 참고만 하기)
